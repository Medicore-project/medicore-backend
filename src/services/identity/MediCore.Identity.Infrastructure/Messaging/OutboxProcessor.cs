...
/// </summary>
/// <remarks>
/// This is the half of the transactional outbox that can fail: an operation commits its event row in
/// the same transaction as the operation, and if the broker is unreachable the row simply stays
    ...
    private async Task ProcessBatchAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var message in messages)
        {
            ...
            if (message.RetryCount >= MaxRetryAttempts)
            {
                ...
                _logger.LogWarning(
                    "Identity outbox message {OutboxMessageId} exceeded max retry attempts. Sent to DLT.",
                    message.Id);
                continue;
            }
