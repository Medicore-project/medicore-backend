...
public sealed partial class CreatePatientRequestValidator : AbstractValidator<CreatePatientRequest>
{
    ...
    private static bool BeAValidNic(string? nic) =>
        !string.IsNullOrWhiteSpace(nic) && NicRegex().IsMatch(PatientRegistrationService.NormalizeNic(nic));

    private static bool BeAValidPhone(string? phone) =>
        !string.IsNullOrWhiteSpace(phone) && PhoneRegex().IsMatch(PatientRegistrationService.NormalizePhone(phone));
